...
import { logsPricingData } from "@/constants/Pricing/index";
...
const Page = () => {
  ...
  const examples =[
    ...
    const logsPriceDisplay:any = logsPricingData[selectedCloud].find((region) => region.region == selectedRegion)
    return (
        <main>
            <div className="ml-[16px] mr-[16px]  md:ml-[42px] md:mr-[80px]">
            <h2 className="text-white text-[20px] font-medium md:font-bold md:text-[24px]">Logs</h2>
            <PricingPlans planName={"Free Plan"}>
                   <p className="text-[12px] mb-[12px] md:text-[14px] text-muted">
            
        You have selected cloud as {selectedCloud} and region as {logsPriceDisplay.region} and cost as {logsPriceDisplay.cost[selectedCurrency]}
          </p>
